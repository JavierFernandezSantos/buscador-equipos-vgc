...
with tab_anadir:
    ...
    if st.button("⚡ Processar y Analizar Pokepaste", type="primary"):
        parsed_pokes = None
        paste_final_url = paste_url_in.strip()
        ...
        if not parsed_pokes:
            ...
            # OPCIÓN 1: Enviar por Webhook automático si está configurado
            if WEBHOOK_URL:
                payload = {
                    "pestaña": reg_target,
                    "owner": owner_in,
                ...
                # OPCIÓN 2: Generar fila lista para copiar y pegar en Excel
                st.subheader("📋 Fila preparada para copiar en Google Sheets:")
                st.info("Copia el texto del cuadro inferior y pégalo como una nueva fila en tu Excel:")
                
                # Formatear valores en orden
                fila_excel_str = f"{code_in}\t{owner_in}\t{desc_in}\t{paste_final_url}\t" + "\t".join(objs_lista) + "\t" + "\t".join(pokes_lista)
                st.code(fila_excel_str)
